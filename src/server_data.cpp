...
int save_config_into_database(Hw_conf hw_conf){
    ...
    curl_es_conf(getElasticSearchAdd(),hw_conf.n_cpu, hw_conf.n_cores, hw_conf.mem_total, hw_conf.n_gpu, hw_conf.n_devices_io, hw_conf.n_interfaces,ip, timestamp);
    free(timestamp);
    free(ip);
	int error = db_insert_CONF(hw_conf.ip_addr_s.c_str(), hw_conf.n_cpu, hw_conf.n_cores, hw_conf.mem_total, hw_conf.n_gpu, hw_conf.n_devices_io, hw_conf.n_interfaces);
...
int log_info_server_data(Hw_conf hw_conf) {
    ...
    unsigned char *buffer = (unsigned char *) calloc(
            (CONF_PACKET_SIZE + 3 * hw_conf.n_gpu + (hw_conf.hostname.length() + 1)) + sizeof(unsigned int),
