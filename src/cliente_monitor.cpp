...
int send_conf_packet(Hw_conf* hw_conf) {
    ...
    int bsent = 0;
    conf_packet = (char *) calloc((CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1)),
                                  sizeof(char));
    if (conf_packet == NULL) {
        ...
        bsent = sendn(sd, conf_packet, (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1)),
                      &server_addr);
        if (bsent != (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1))) {
            std::cerr << "Error sending configuration packet." << endl;
            error = -1;
        }

        //TMR with other 2 servers
        bsent = sendn(sd1, conf_packet, (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1)),
                      &server_bk_1);
        if (bsent != (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1))) {
            std::cerr << "Error sending configuration packet." << endl;
            error = -1;
        ...
        bsent = sendn(sd2, conf_packet, (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1)),
                      &server_bk_2);
        if (bsent != (CONF_PACKET_SIZE + 3 * hw_conf->n_gpu + (hw_conf->hostname.length() + 1))) {
            std::cerr << "Error sending configuration packet." << endl;
            error = -1;
