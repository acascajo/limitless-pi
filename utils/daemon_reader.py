...
def obtainHWinfo(ipaddress, inputfile):
    global Hwcollection
    Hwconf = namedtuple("Hwconf",
                        "node_name total_memory num_cpus num_cores num_io_devices num_network_interfaces num_gpus gpu_compat memory_gpu capa_gpu")
    with open(inputfile, 'rb') as ifile:
        ...
        while ipfound:
            ...
            Hwcollection[ip_decimal] = Hwconf(name_node, mem_total, numcpus, numcores, num_io, num_net, num_gpu,
                                              gpu_comp, gpu_mem, gpu_capa)
            ...
            try:
                ...
                for i in range(0, n_samples):
                    ...
                    if sig == 0:
                        ...
                        time_tot = datetime.timedelta(seconds=(time_interval * i)) + datetime.datetime.fromtimestamp(
                            timedec)
                        """
...
def create_graph(ip, hwconf):
    ...
    print filename
    num_columns = 5 + hwconf.num_io_devices + hwconf.num_network_interfaces + (3 * hwconf.num_cpus) + (
            4 * (hwconf.num_gpus))
    ...
    for i in range(0, hwconf.num_cpus):
        c = next(color)
        enertmp, = plt.plot_date(data['Time'], data["CPU Energy" + str(i) + " J"], c=c, fmt='-',
                                 label='CPU ' + str(i) + '(J)')
        ener.append(enertmp)
...
def main(argv):
    ...
    try:
        ...
        print 'daemon_reader.py -i <IP/a> -f <daemon_log_file> -c <hw_conf_file> [-g <generategraph>]'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print 'daemon_reader.py -i <IP/a> -f <daemon_log_file> -c <hw_conf_file> [-g <generategraph>]'
